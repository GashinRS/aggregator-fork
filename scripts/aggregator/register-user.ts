...
const REGISTRATION = `${config.aggregatorServer}/registration`;
const AS_URL = config.asServer;
...
async function main() {
  ...
  const { state, interval = 5, verification_uri, verification_uri_complete, user_code } = startData;
  ...
  while (true) {
    ...
    const finishResp = await fetch(REGISTRATION, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ registration_type: "device_code", state }),
    });

    if (finishResp.status === 202) { process.stdout.write("."); continue; }
    ...
    if (finishResp.status === 400) throw new Error(`Device flow failed: ${await finishResp.text()}`);
    throw new Error(`Unexpected response: ${finishResp.status}`);
  }
}

await main().catch(err => { console.error("Error:", err.message); });
